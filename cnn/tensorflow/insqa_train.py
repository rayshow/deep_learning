...
from insqa_w2v import InsQACNN_W2V
import operator
...
#train and test vocab
vocab = insurance_qa_data_helpers.build_vocab()

#answer list
alist = insurance_qa_data_helpers.read_alist()

#traint question/answer items
raw = insurance_qa_data_helpers.read_raw()

# random batch size: quastions, postive answers, negtive answers
x_train_1, x_train_2, x_train_3 = insurance_qa_data_helpers.load_data_6(vocab, alist, raw, FLAGS.batch_size)

# test line, voc/vec dictionary
testList, vectors = insurance_qa_data_helpers.load_test_and_vectors()

vecab_embedding, voc_array = insurance_qa_data_helpers.load_embedding(vocab, vectors)

vocab_len = len(vocab)
print(len(vecab_embedding))
print(len(vecab_embedding[0]))
#print(voc_array[1], vecab_embedding[1])
#print(voc_array[vocab_len-1], vecab_embedding[vocab_len-1])
...
print("Load done...")
print("evaluate :", FLAGS.evaluate_every);

val_file = '../../insuranceQA/test1.sample'
precision = '../../insuranceQA/acc'
...
with tf.Graph().as_default():
  with tf.device("/cpu:1"):
    ...
    with sess.as_default():
        ...
        embedding_vocab =  np.random.rand(vocab_size, embedding_size);
