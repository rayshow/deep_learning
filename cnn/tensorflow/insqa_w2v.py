...
class InsQACNN_W2V(object):
    def __init__(
      ...
      filter_sizes, num_filters, l2_reg_lambda=0.0):
        ...
        self.embedding = tf.placeholder(tf.float32, [vocab_size, embedding_size], name="embedding_lookup")
        ...
        with tf.device('/cpu:0'), tf.name_scope("embedding"):
            ...
            chars_1 = tf.nn.embedding_lookup(self.embedding, self.input_x_1)
            chars_2 = tf.nn.embedding_lookup(self.embedding, self.input_x_2)
            chars_3 = tf.nn.embedding_lookup(self.embedding, self.input_x_3)
        ...
        # expanded is [batch_size, seq_size, embedding_length, 1]
        ...
        for i, filter_size in enumerate(filter_sizes):
            with tf.name_scope("conv-maxpool-%s" % filter_size):
                # num_filters = 500, filter_size = 1,2,3,5,embedding_size = 50
        ...
        pooled_multiply_12 = tf.reduce_sum(tf.multiply(pooled_flat_1, pooled_flat_2), 1) #计算向量的点乘Batch模式
        pooled_multiply_13 = tf.reduce_sum(tf.multiply(pooled_flat_1, pooled_flat_3), 1)

        with tf.name_scope("output"):
            self.cos_12 = tf.divide(pooled_multiply_12, tf.multiply(pooled_len_1, pooled_len_2), name="scores") #计算向量夹角Batch模式
            self.cos_13 = tf.divide(pooled_multiply_13, tf.multiply(pooled_len_1, pooled_len_3))
             
        zero = tf.constant(0, shape=[batch_size], dtype=tf.float32)
        margin = tf.constant(0.00, shape=[batch_size], dtype=tf.float32)
