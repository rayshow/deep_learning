...
def load_embedding(vocab, vectors):
    embedding = []
    vocab_array = []
    index = 0
    vector_len = len(vectors['</s>'])
    for k,v in vocab.items():
        vocab_array.append(k);
        if k in vectors:
            embedding.append(vectors[k])
        else:
            print(k,"not found in embedding")
            empty = []
            for i in range(0,vector_len):
                empty.append(0.0)
            embedding.append(empty)
    matrix = np.array(embedding)
    return matrix, vocab_array
